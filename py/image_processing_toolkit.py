...
def lab_matrix_to_bgr_image(lab_matrix):
    """Returns image in BGR space representation based on CIELab space matrix.

    matrix: CIELab matrix
    """
    bgr_matrix = cv2.cvtColor(lab_matrix, cv2.COLOR_LAB2BGR)
    # clip any values that fall outside the range [0, 1]
    bgr_matrix = np.clip(bgr_matrix, 0, 1)

    # the current colorized image is represented as a floating point
    # data type in the range [0, 1] -- let's convert to an unsigned
    # 8-bit integer representation in the range [0, 255]
    colorized_rgb_image = (255 * bgr_matrix).astype("uint8")
...
def merge_l_channel_with_ab_space(l_channel, ab_space):
    """Merge l_channel with ab channel to single matrix - image.
    """
